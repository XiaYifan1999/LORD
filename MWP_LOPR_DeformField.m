function [T_fin,C]=MWP_LOPR_DeformField(s1_shape,s2_shape,k,T0,Nf,num_iters,tau)
...
%     mplot_mesh_rgb([s1_shape.surface.X,s1_shape.surface.Y,s1_shape.surface.Z],s1_shape.surface.TRIV,rgb(Tn(:,1),:)); % title('Current');
%     
    [T,M_num] = LPO_DeformField(Tn,s1_shape,s2_shape,neighbors1,neighbors2,n2,tau);
...
%     rgb=coord2rgb([s2_shape.surface.X,s2_shape.surface.Y,s2_shape.surface.Z]);
%     mplot_mesh_rgb([s1_shape.surface.X,s1_shape.surface.Y,s1_shape.surface.Z],s1_shape.surface.TRIV,rgb(T(:,1),:)); title('Current');
%     
...
T_fin = fast_pMap_NNinterp(T_fin,s1_shape);
